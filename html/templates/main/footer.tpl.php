</main><?php //.main-center?>

<footer class="main-footer">
    <div class="layout-main">
        <div class="footer-wrap">
            <div class="footer-left">
                <div class="footer-block">
                    <a href="#" class="footer-logo">
                        <img class="footer-img-logo" src="/skin/images/Group%20707.svg" alt=""/>
                    </a>
                    <div class="footer-copy">Copyright © 2021 BAY MASTER APPLIANCE</div>
                </div>
                <div class="footer-block">
                    <a href="tel:+4084844353" class="footer-phone">(408) 484-4353</a>
                    <a href="mailto:ask@service@bay-master-appliance.com" class="footer-mail">service@bay-master-appliance.com</a>
                </div>
            </div>
            <div class="footer-right">
            <nav class="footer-menu">
                <div class="footer-menu-block">
                    <li class="footer-menu-el _hasSubmenu">
                        <a href="" class="footer-menu-link">
                            Services
                            <svg width="10" height="8" viewBox="0 0 10 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M5 8L0.669875 0.5L9.33013 0.500001L5 8Z" fill="white"/>
                            </svg>
                        </a>
                        <ul class="header-submenu">
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Refrigerators</a>
                            </li>
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Ovens | Microwave</a>
                            </li>
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Ranges | Cooktops</a>
                            </li>
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Dishwashers</a>
                            </li>
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Washers</a>
                            </li>
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Dryers</a>
                            </li>
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Stackable</a>
                            </li>
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Garbage disposals</a>
                            </li>
                        </ul>
                    </li>
                    <li class="footer-menu-el _hasSubmenu">
                        <a href="" class="footer-menu-link">
                            About us
                            <svg width="10" height="8" viewBox="0 0 10 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M5 8L0.669875 0.5L9.33013 0.500001L5 8Z" fill="white"/>
                            </svg>
                        </a>
                        <ul class="header-submenu _one_el">
                            <li class="header-submenu__el">
                                <a href="/" class="header-submenu__link">Reviews</a>
                            </li>
                        </ul>
                    </li>
                </div>
                <div class="footer-menu-block">
                    <li class="footer-menu-el">
                        <a href="" class="footer-menu-link">
                            Contacts
                        </a>
                    </li>
                    <li class="footer-menu-el">
                        <a href="" class="footer-menu-link">
                            Areas
                        </a>
                    </li>
                </div>
            </nav>
            <button class="footer-button">Request services</button>
        </div>
        </div>
    </div>
</footer>

<div id="review-popup" style="display: none;">
	<?php include($_SERVER['DOCUMENT_ROOT'] . "/html/templates/review.tpl.php") ?>
</div>

</div><?php //.main-container?>
<?php include($_SERVER['DOCUMENT_ROOT'] . "/html/templates/footer.tpl.php") ?>
